...
The core accepts a dictionary word first, then exactly one changed position per accepted word, without repeats. Streak is accepted chain length. There is no timer. At that baseline, progress lived in Livewire state and signed-in history was not persisted. The September 9 source match and current account saves are described below. `sifrious/edge` is a later prototype with runs, plays, and anonymous sync; its unavailable path dependency and old no-account assumptions must not be treated as a deployed contract.
...
The MME-3959 branch now implements metadata, replay validation, account-owned run uploads, safe retries, and server-rendered browser play with explicit saving and resume. The shared Accounts Client verifies tokens with mary.is. See `docs/four-letter-words-api.md`. The canonical module now exports a deterministic Composer artifact for the local standalone native prototype. Device builds and native authentication remain pending. GitHub Socialite and Passport are already deployed at mary.is; registration remains disabled while outbound mail is unconfigured.

Tracking: https://linear.app/sifirous/issue/MME-2369 and https://linear.app/sifirous/issue/MME-3949.

## September 9 deployment and native prototype

mary.win deployed `818a422` through Cloud deployment `depl-a2b3b3f0-8a21-4f2c-9a48-fd1ab51875e8`. The existing tile game and `/games/four-letter-words/play` remain available. The latter is a complete HTML play path with explicit account saving and resume. Database sessions and secure cookies are enabled. The game API rejects unauthenticated saved-run reads and writes.

mary.is deployed `1893ab5` through Cloud deployment `depl-a2b3b32d-547d-457c-8c8a-b552d24cc083`. GitHub sign-in reached the Four Letter Words consent page in the production browser. Consent and a real account save have not yet been completed. Local email verification and reset notifications now route to `Account::local_email`; real outbound mail remains unconfigured and registration stays disabled.

Verification: mary.win has 193 passing PHP tests with 755 assertions and one SQLite-only concurrency skip, five JavaScript tests, and a successful asset build. The saved-run suite and a competing-process write test also passed against PostgreSQL. mary.is has 74 passing tests with 534 assertions and one SQLite-only skip. Neither PostgreSQL verification nor PHP native-component tests substitute for device testing.

The dictionary matches the BSD `web2` source byte for byte under the documented filter. See `packages/four-letter-words/docs/DICTIONARY.md`. No game words changed.

The standalone `four-letter-words` checkout has a SuperNative screen, number selection, local run persistence, and component tests. The app consumes the archive generated by `packages/four-letter-words/bin/export-package.py`; the rules and dictionary remain authored here. Native authentication needs secure-storage plugin access, an exact registered native return link, and device verification. Full Xcode and the Android SDK are absent from this Mac.
